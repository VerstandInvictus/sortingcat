...
# sourced from:
# https://raw.githubusercontent.com/hydrusnetwork/hydrus/master/include/HydrusTagArchive.py

# Please feel free to use this file however you wish.
# None of this is thread-safe, though, so don't try to do anything clever.


# If you want to make a new tag archive for use in hydrus, you want to do
# something like:
...
# hta.BeginBigJob()
# for ( hash, tags ) in my_complex_mappings_generator: hta.AddMappings(
# hash, tags )
# -or-
...
# If you are only adding a couple tags, you can exclude the BigJob stuff. It
#  just makes millions of sequential writes more efficient.
...
# Then go hash = hash.decode( 'hex' ) before you pass it to
# Add/Get/Has/SetMappings


# If you have tags that are namespaced like hydrus (e.g. series:ghost in the
#  shell), then check out:
# GetNamespaces
# DeleteNamespaces
# and
# RebuildNamespaces

# RebuildNamespaces takes namespaces_to_exclude, if you want to curate your
# namespaces a little better.

# If your GetNamespaces gives garbage, then just hit DeleteNamespaces. I'll
# be using the result of GetNamespaces to populate
# the tag import options widget when people sync with these archives.


# And also feel free to contact me directly at hydrus.admin@gmail.com if you
#  need help.
...
class HydrusTagArchive(object):
    def __init__(self, path):
        ...
        self._namespaces = {namespace for (namespace,) in self._c.execute(
    ...
    def _AddMappings(self, hash_id, tag_ids):
        ...
        self._c.executemany(
            'INSERT OR IGNORE INTO mappings ( hash_id, tag_id ) VALUES ( ?, '
            '? );',
            ((hash_id, tag_id) for tag_id in tag_ids))
    ...
    def _InitDB(self):
        ...
        self._c.execute(
            'CREATE TABLE hashes ( hash_id INTEGER PRIMARY KEY, '
            'hash BLOB_BYTES );')
        self._c.execute(
            'CREATE UNIQUE INDEX hashes_hash_index ON hashes ( hash );')

        self._c.execute(
            'CREATE TABLE mappings ( hash_id INTEGER, tag_id INTEGER, '
    ...
    def _GetHashId(self, hash, read_only=False):
        ...
        result = self._c.execute('SELECT hash_id FROM hashes WHERE hash = ?;',
        ...
        if result is None:
            ...
            self._c.execute('INSERT INTO hashes ( hash ) VALUES ( ? );',
                            (sqlite3.Binary(hash),))
    ...
    def _GetTagId(self, tag):
        ...
        result = self._c.execute('SELECT tag_id FROM tags WHERE tag = ?;',
                                 (tag,)).fetchone()
    ...
    def DeleteMapping(self, hash, tag):
        ...
        self._c.execute(
            'DELETE FROM mappings WHERE hash_id = ? AND tag_id = ?;',
            (hash_id, tag_id))
    ...
    def GetHashType(self):
        ...
        try:
            (hash_type,) = self._c.execute(
                'SELECT hash_type FROM hash_type;').fetchone()
        except:

            try:

                (hash,) = self._c.execute(
                    'SELECT hash FROM hashes;').fetchone()

                if len(hash) == 16:
                    self.SetHashType(HASH_TYPE_MD5)
                elif len(hash) == 20:
                    self.SetHashType(HASH_TYPE_SHA1)
                elif len(hash) == 32:
                    self.SetHashType(HASH_TYPE_SHA256)
                elif len(hash) == 64:
                    self.SetHashType(HASH_TYPE_SHA512)
                else:
                    raise Exception()
                ...
                raise Exception(
                    'This archive has no hash type set, and as it has no '
                    'files, no hash type guess can be made.')
    ...
    def GetTags(self, hash):
        ...
        result = {tag for (tag,) in self._c.execute(
            'SELECT tag FROM mappings, tags USING ( tag_id ) WHERE hash_id = '
            '?;',
            (hash_id,))}
    ...
    def IterateMappings(self):
        ...
        hash_ids = [hash_id for (hash_id,) in
                    self._c.execute('SELECT hash_id FROM hashes;')]

        for hash_id in hash_ids:

            (hash,) = self._c.execute(
                'SELECT hash FROM hashes WHERE hash_id = ?;',
                (hash_id,)).fetchone()
    ...
    def RebuildNamespaces(self, namespaces_to_exclude=set()):
        ...
        for (tag,) in self._c.execute('SELECT tag FROM tags;'):
            ...
            if ':' in tag:
                ...
                if namespace != '' and namespace not in self._namespaces and\
                                namespace not in namespaces_to_exclude:
                    self._namespaces.add(namespace)
    ...
    def SetHashType(self, hash_type):
        ...
        self._c.execute('INSERT INTO hash_type ( hash_type ) VALUES ( ? );',
                        (hash_type,))
